...
/** scale+crop que normaliza qualquer entrada pra 9:16 — igual no frame e no vídeo. */
...
export async function extractMiddleFrame(
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ffmpeg(srcPath)
        .seekInput(middle)
        .outputOptions(["-frames:v 1", `-vf ${NORMALIZE_9x16}`, "-q:v 3"])
        .on("error", reject)
        .on("end", () => resolve())
        .save(outPath);
    });
...
async function buildOverlayCardPng(
  ...
  const box = `<rect x="${SAFE_X}" y="0" width="${CARD_WIDTH}" height="${boxHeight}" rx="${CARD_RADIUS}" fill="#000000" fill-opacity="0.5"/>`;
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${VIDEO_WIDTH}" height="${totalHeight}">${box}${rendered.svg}</svg>`;
...
/** Y do overlay ao longo do tempo: desliza de baixo na entrada, depois fica parado. */
function yExpr(restY: number): string {
  const slideProgress = `min(1,max(0,(t-${FADE_IN_START})/${FADE_IN_END - FADE_IN_START}))`;
  return `${restY}+(1-(${slideProgress}))*${SLIDE_DISTANCE}`;
}
...
export async function renderVideoWithAnimatedTitle(
  ...
  const card = await buildOverlayCardPng(params.title || "", params.logoUrl);
  ...
  const filterComplex = [
    `[0:v]${NORMALIZE_9x16}[main]`,
    `[1:v]format=rgba,fade=t=in:st=${FADE_IN_START}:d=${FADE_IN_END - FADE_IN_START}:alpha=1,fade=t=out:st=${HOLD_END}:d=${FADE_OUT_END - HOLD_END}:alpha=1[txt]`,
